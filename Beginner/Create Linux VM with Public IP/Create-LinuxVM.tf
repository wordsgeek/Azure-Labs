...
resource "azurerm_public_ip" "pip" {
  name                = "LinDemoVM-PIP"                           # Name of the Public IP
...
resource "azurerm_network_interface" "VMNic" {
  name                = "LinDemoVM-nic"                    # Name of the Network Interface
...
resource "azurerm_linux_virtual_machine" "LinVM" {
  ...
  disable_password_authentication = "false"                    # By default its set to true in that case we need to use key and pair to login.
  admin_username      = "adminuser"                            # Virtual Machine Admin Username
  admin_password      = "P@$$w0rd1234!"                        # Virtual Machine Admin Password
  custom_data         = file("cloud-initbase64.txt")           # Add the Custom Data
